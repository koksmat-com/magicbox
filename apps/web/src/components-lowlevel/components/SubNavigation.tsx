...
import { NavigationNode, onOver } from '../helpers';


export const SubMenuItem = (props: { selectParent: () => void,node: NavigationNode, onNavigate: () => void }): JSX.Element => {
   ...
   // const [children, setChildren] = useState<NavigationNode[]>(props.node.Children)
    ...
    return (<div style={{ display: "flex" }}
        onMouseOver={() => {
            ...
            setHover(true)
            if (props.node && props.node.onOver) props?.node?.onOver(props.node)
        }}
        onMouseLeave={() => {
    ...
    >
        ...
        {(props && props.node && props.node.Children && props?.node?.Children?.length > 0) &&
...
export const SubNavigation = (props: { selectParent: () => void, level: number, node: NavigationNode | undefined | null,  onNavigate: () => void }): JSX.Element => {
    const [selectedNavigationNode, setselectedNavigationNode] = useState<NavigationNode>()
    const [selectedNavigationNodeChilds, setselectedNavigationNodeChilds] = useState<NavigationNode>()
    const [showLevel3, setShowLevel3] = useState(false)
    const [showLevel4, setShowLevel4] = useState(false)

    const [level4, setLevel4] = useState<NavigationNode[]>([])

    const Url  = props.node?.Url as string
    ...
    return (
        <div
            ...
            <div style={{ width: "300px" }}>
                ...
                {props && props.node && props.node.Children &&  props?.node.Children.map((childNode: NavigationNode, key) => {
                    childNode.onOver = onMouseOver
                    return <SubMenuItem selectParent={select} key={key} node={childNode} onNavigate={props.onNavigate} />;
                })}

            </div>
            {showLevel3 && selectedNavigationNode && selectedNavigationNode.Children &&  selectedNavigationNode?.Children?.length > 0 &&
                <div>

                    <SubNavigation selectParent={select} level={props.level + 1} onNavigate={props.onNavigate} node={selectedNavigationNode} />
                </div>
            }
            {showLevel4 &&
                <div>

                    <SubNavigation selectParent={select} level={props.level + 1}  onNavigate={props.onNavigate} node={selectedNavigationNodeChilds} />
                </div>
            }
        </div>
    );
