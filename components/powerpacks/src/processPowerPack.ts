...
export async function processPowerPack(
  powerpack: IEndPointHandler,
  rawInput: Request,
  returnPreview?: boolean
): Promise<IResult<any>> {
  const log = debug(
    "magicbox:businesslogic:scripts-exchange:sharedmailbox:create"
  );
  ...
  let script = `
    ...
    powerbrick ${powerpack.script.commandParameters}
  ...
  //onProcessed
  const outputParse = powerpack.output.safeParse(result.data);
