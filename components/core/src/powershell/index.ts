...
export interface IPowerShellResult {
  success: string[];
  error: string[];
  warning: string[];
  verbose: string[];
...
export function executePowerShell(command: string): Promise<IPowerShellResult> {
  const log = debug("magicbox.powershell:executePowerShell");
  return new Promise(async (resolve, reject) => {
    const shell = new PowerShell();
    shell.debug$.subscribe((data) => {
      log("debug", data);
    });
    shell.error$.subscribe((data) => {
      log("error", data);
    });
    shell.warning$.subscribe((data) => {
      log("warning", data);
    });
    shell.verbose$.subscribe((data) => {
      log("verbose", data);
    });
    shell.info$.subscribe((data) => {
      log("info", data);
    });
    shell.success$.subscribe((data) => {
      log("success", data);
    });

    shell.call(command).subscribe((result: IPowerShellResult) => {
